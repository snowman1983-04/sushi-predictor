...
### 🌐 ライブデモ：[https://sushi-predictor-snowman01.streamlit.app/](https://sushi-predictor-snowman01.streamlit.app/)
...
[https://sushi-predictor-snowman01.streamlit.app/](https://sushi-predictor-snowman01.streamlit.app/)
...
- **期間**：2024-04-01 〜 2027-03-31（1,095 日、AIエンジニアレビュー反映で 3 年に拡張）
- **商品数**：4 種類（P001〜P004、各商品の販売スケジュールは [`docs/model_comparison.md`](./docs/model_comparison.md) 参照）
- **総レコード数**：4,380 行
- **生成元**：[`data/generate_data.py`](./data/generate_data.py)（乱数シード固定＝42、再現可能）

カラム：`date, product_id, product_name, day_of_week, is_weekend, is_holiday, is_pension_day, is_sale_day, weather, temperature, precipitation, effective_price, cpi_index, sales_count`
...
### 回帰モデル（CV MAE：小さいほど良い、v1.1）

| 商品 | 線形回帰 | ランダムフォレスト | LightGBM |
|---|---:|---:|---:|
| P001 水木寿司（10貫） | 5.49 | **1.61** | 2.10 |
| P002 春8貫寿司         | **1.80** | 1.79 | 2.00 |
| P003 絆10貫寿司        | 2.18 | **2.08** | 2.29 |
| P004 ランチ寿司         | 4.14 | **1.87** | 2.34 |

線形回帰の P004 は CV R²=0.19 と破滅的（`is_sale_day` と販売可能日の交互作用が線形では表現不可）。RF が圧勝。

### 分類モデル（ロジスティック回帰、3クラス：少/普/多、v1.1）
